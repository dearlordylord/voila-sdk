...
The CLI opens a persistent Playwright profile at `~/.cache/voila/browser-profile` by default. Override it with `--profile <dir>`. Log in manually, then close the browser window to save. The command requires authenticated cookie evidence, validates the capture with Voila inside the guarded session-file cycle, and saves only the active snapshot returned by that check. Inactive or unauthorized captures leave the existing file untouched.
