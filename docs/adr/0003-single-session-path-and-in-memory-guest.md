...
- **A login validates before its guarded write.** `persistValidatedLoginSession` runs session health inside the guarded cycle, persists only an active snapshot, and maps a dropped write to `VoilaAuthSessionSuperseded` instead of retrying over the newer lineage that won.
