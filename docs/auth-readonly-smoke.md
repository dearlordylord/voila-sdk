...
The snapshot path must point to JSON produced by `voila auth login`. Keep it outside the repository, or use a path under `local-session-snapshots/`, which is ignored by git.
