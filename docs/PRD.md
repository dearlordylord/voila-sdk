...
- The repo contains the intended quality harness, including strict TypeScript, Effect Schema, type-aware Oxlint and Effect diagnostics, Oxc/dprint formatting, Vitest coverage with an independent summary verifier, property test placement rules, cyclomatic-complexity and package-boundary checks, duplication checks, circular dependency checks, bounded quality-stage execution, and Husky pre-commit hooks.
