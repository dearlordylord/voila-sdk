...
type LiveIdentitySmokeFailure =
  ...
  | {
    readonly _tag: "LiveIdentityHomepageFailed"
    readonly status?: number
  }
  | {
    readonly _tag: "LiveIdentityUserAgentMismatch"
    readonly requestCount: number
  }
  | {
    readonly _tag: "LiveIdentityFixtureBootstrapFailed"
...
const runSmoke = async (): Promise<Either.Either<undefined, LiveIdentitySmokeFailure>> => {
  ...
  const homepageResponse = await environment.right.transport.request({
    headers: {},
    method: "GET",
    url: voilaHomepage
  })
  ...
  const blocked = await searchProducts(bootstrap.right.session, {
    pageSize,
    query: harmlessQuery
  }, blockedTransport)
