...
The session-domain wrapper over [`@firfi/cas-file-store`](../cas-file-store): the only way a session snapshot reaches disk.

Draft package: private to this workspace, not published.

## Why

A session snapshot read at boot and written back later is a blind write: a process that loaded the file before an interactive login silently reverts it on its next tick. `@firfi/cas-file-store` owns the mechanics of the read-modify-write cycle; this package states it in session terms, so callers work in snapshots rather than bytes and the session-specific safety rules live in one place.
